...
abstract class Field extends FormerObject implements FieldInterface
{
	...
	public function wrapAndRender()
	{
        ...
        $html = $this->autoWrapFormGroup($html);
		return $html;
	}

    /**
     * 自动外包form-group样式
     * 主要解决在模板中调用Component时，因解析顺序错误造成的表单无法自动生成form-group问题
     * @param $html
     * @return string
     */
	private function autoWrapFormGroup($html)
    {
        if($this->app['former']->getOption('auto_wrap_form_group') == false){
            return $html;
        }
        if(in_array($this->element, ['button']) || in_array($this->type, ['hidden'])){
            return $html;
        }
        if(!Str::contains($html, 'form-group')){
	...
	{
        $text = $this->autoMarkRequired($text);
     ...
     * @return string
     */
	private function autoMarkRequired($text)
    {
        if($this->app['former']->getOption('auto_mark_required') == false){
            return $text;
        }
        $rules = rules();
        if(isset($this->label)){
            if(isset($rules[$this->name]) && Str::contains($rules[$this->name], 'required')){
                $text .= $this->app['former']->getOption('required_text');
            }else{
                $text .= $this->app['former']->getOption('nullable_text');
            }
        }
        return $text;
    }
